...
const FolderPage = ({ page, pageIndex, folderIndex, updatePage, deleteFolderPage }) => {
  ...
  const handleSave = useCallback(() => {
    updatePage(folderIndex, pageIndex, { ...page, title, content });
  }, [folderIndex, pageIndex, page, title, content, updatePage]);
  ...
  const handleDelete = useCallback(() => {
    if (window.confirm("삭제하시겠습니까?")) {
      deleteFolderPage(folderIndex, pageIndex);
      navigate(`/folder/${folderIndex + 1}`);
    }
  }, [deleteFolderPage, folderIndex, pageIndex, navigate]);
...
FolderPage.propTypes = {
  ...
  deleteFolderPage: PropTypes.func.isRequired,
};
